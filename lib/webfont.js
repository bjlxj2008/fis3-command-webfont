...
module.exports.generateFonts = function(options) {
    ...
    files = _.filter(files, isSvgFile).map(function(file) {
        return file;
    });
    ...
    var o = {
        ...
        types: optionToArray(options.types, 'svg,eot,woff,ttf'),
        order: optionToArray(options.types, wf.fontFormats),
    ...
    function readTemplate(template, syntax, ext, optional) {
        var filename = template ? path.resolve(template.replace(path.extname(template), ext)) : path.join(__dirname, 'templates/' + syntax + ext);
        if (fs.existsSync(filename)) {
            return {
                filename: filename,
            ...
            return fis.log.error('Cannot find template at path: ' + filename);
        }
    }
